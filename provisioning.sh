...
aws s3 cp s3://${poc_bucket}/ /opt/springboot-s3-example/ --region=${region} --recursive --exclude "*" --include "springboot-s3-example*.jar"
...
RUN_ARGS="--spring.datasource.url=jdbc:mysql://${database_endpoint}/${database_name}?useSSL=false --spring.datasource.username=${database_username} --spring.datasource.password=${database_password}"
EOF
